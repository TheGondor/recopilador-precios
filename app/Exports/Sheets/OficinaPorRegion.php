...
class OficinaPorRegion implements FromView, WithTitle, ShouldAutoSize, WithStyles, WithEvents
{
    private $region;
    private $provider;
    private $count;
    ...
    public function view(): View
    {
        ...
        $products = Product::where('convenio', 'Oficina')->join('product_providers', function($query) use ($provider){
        ...
        $this->count = Product::where('convenio', 'Oficina')->join('product_providers', function($query) use ($provider){
            $query->on('products.id', '=', 'product_providers.product_id');
            $query->join('providers', function($query) use ($provider){
                $query->on('providers.id', '=', 'product_providers.provider_id');
                $query->where('providers.id', $provider);
